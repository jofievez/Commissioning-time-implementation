...
class AmplObject:
    ...
    def __init__(self, mod_1_path, mod_2_path, data_path, options, type_model = 'MO',ampl_path=None):
        ...
        self.results = dict.fromkeys(['TotalCost', 'C_inv_phase', 'C_inv_phase_tech',
                                      ...
                                      'F_decom', 'Decision_tracking', 'Sto_assets', 'Year_balance',
    ...
    def get_total_cost(self):
        ...
        c_inv_phase['Phases'] = pd.Categorical(c_inv_phase['Phases'],['2019_2020'] + self.sets['PHASE_UP_TO'])
        ...
        phases = sorted(set(['2019_2020'] + self.sets['PHASE_UP_TO'] + self.sets['PHASE_WND']))
        ...
        # self.results['Transition_cost'] = new_transition_cost
    ...
    def get_total_gwp(self):
        ...
        # gwp_limit.index.set_names('Years', inplace=True)
        # gwp_limit.columns = ['Year', 'Value']  # Rename columns
        # gwp_limit = gwp_limit.set_index('Year')  # Set index
        # # Add the new row
        # gwp_limit.loc['YEAR_2015'] = np.inf
        # # Sort index to keep chronological order, if needed
        # gwp_limit = gwp_limit.sort_index()
    ...
    def get_gwp_breakdown(self):
        ...
        """
        gwp_breakdown = gwp_constr.merge(gwp_op,left_index=True, right_index=True, how='outer').reset_index()
    ...
    def get_new_old_decom(self):
        ...
        # set Years and Technologies as categorical data and sort it
        assets['Phases'] = pd.Categorical(assets['Phases'], ['2019_2020']+self.sets['PHASE_UP_TO'])
    ...
    def get_decision_tracking(self):
        logging.info('Getting Decision_tracking')

        F_decided = self.get_elem('F_decided_up_to')
        f_decided_index = F_decided.index.names
        F_decided = F_decided.rename_axis(index={f_decided_index[0]: 'Phases'}, axis=1)

        F_decided_realized = self.get_elem('F_decided_realized_up_to')
        f_decided_realized_index = F_decided_realized.index.names
        F_decided_realized = F_decided_realized.rename_axis(index={f_decided_realized_index[0]: 'Phases'}, axis=1)

        decision_tracking = F_decided.merge(
            F_decided_realized,
            left_index=True,
            right_index=True,
            how='outer'
        ...
        decision_tracking['Phases'] = pd.Categorical(decision_tracking['Phases'], self.sets['PHASE_UP_TO'])
        decision_tracking = decision_tracking[decision_tracking['Phases'].notna()]
        self.categorical_esmy(df=decision_tracking, col_name='Technologies', el_name='Technologies')
        decision_tracking.sort_values(by=['Phases', 'Technologies'], axis=0, ignore_index=True, inplace=True)
        decision_tracking.set_index(['Phases', 'Technologies'], inplace=True)

        threshold = 0
        decision_tracking = decision_tracking.mask((decision_tracking > -threshold) & (decision_tracking < threshold), np.nan)
        decision_tracking.dropna(how='all', inplace=True)

        self.results['Decision_tracking'] = decision_tracking
        return

...
class AmplPreProcessor:
    ...
    def remaining_update(self,i,file_in = 'PES_data_remaining.dat', file_out = 'PES_data_remaining_wnd.dat'):
        ...
        curr_phases_up_to = ['2019_2020'] + self.phases_up_to[i]
        curr_phases_wnd = self.phases_opti[i]
        ...
        if n_year_overlap > 0 and curr_phases_wnd[-1] != last_phase:
            n_phase_extra = int(n_year_overlap/self.t_phase)
        ...
        with open(pth_file,encoding='utf-8') as fp:
            ...
            for line in fp:
                ...
                break
            if n is None:
                raise ValueError(f'Error: no technology rows found in {pth_file}')
        with open(os.path.join(pth_model,file_out),'w+', encoding='utf-8') as f:
            ...
            with open(pth_file,encoding='utf-8') as fp:
                next(fp)
                for line in fp:
                    list_line = re.split(r'\s+', line.strip())
                    if not list_line or list_line[0] == ';':
                        continue
                    if len(list_line) < n_phase + 1:
                        break

                    tech_name = list_line[0]
                    remaining_values = list_line[-n_phase:]
                    f.write(tech_name + '\t' + '\t'.join(remaining_values) + '\n')
            f.write('; \n')
    ...
    def pathway_window(self):
        ...
        if years_window % self.t_phase != 0:
            raise ValueError(f'Error : the number of years in the window of optimisation is not correct.\nIt should be a multiple of {self.t_phase}')
        ...
        if years_overlap % self.t_phase != 0:
            raise ValueError(f'Error : the number of years of overlap is not correct.\nIt should be a multiple of {self.t_phase}')
    ...
    @staticmethod
    def without_2019(self):
        self.list_year.remove('YEAR_2019')
        self.list_phase.remove('2019_2020')
...
class AmplGraph:
    ...
    def __init__(self, pkl_file, ampl_obj,case_study):
        ...
        self.x_axis = [2020, 2022, 2024, 2026, 2028, 2030, 2032, 2034, 2036, 2038, 2040, 2042, 2044, 2046, 2048, 2050]
    ...
    def graph_layer(self, ampl_collector = None, plot = True):
        ...
        for k in results.columns:
            ...
            if plot:
                ...
                if k in ['GAS','H2','WOOD','WET_BIOMASS','CO2_ATM','CO2_CAPTURED', 'CO2_INDUSTRY']:
                    EUD_2020 = 0
                    EUD_2050 = 0
                else:
    ...
    def graph_gwp_constr_phase(self):
        pio.renderers.default = 'browser'

        # --- Load results ---
        # On reprend la source qui contient le GWP de construction
        results = self.ampl_collector['Gwp_breakdown'].copy()

        # On ne garde que la colonne pertinente
        if 'GWP_op' in results.columns:
            results = results.drop(columns=['GWP_op'])
        ...
        # ---------- GWP Construction per technology ----------
        # On reset l'index pour avoir 'Years' et 'Elements' en colonnes
        df_gwp = results.copy().reset_index()
        
        # Nettoyage du nom des années pour qu'elles ressemblent à des phases
        df_gwp['Years'] = df_gwp['Years'].astype(str).str.replace('YEAR_', '')
        
        # On renomme pour coller à votre logique 'Phases'
        df_gwp = df_gwp.rename(columns={
            'GWP_constr': 'GWP', 
            'Years': 'Phases',
            'Elements': 'Category'
        })
        
        df_gwp['Type'] = 'GWP Construction'
        
        # On ajoute au tableau final
        dfs.append(df_gwp[['Phases', 'GWP', 'Category', 'Type']])

        # ---------- Concatenate ----------
        df_to_plot = pd.concat(dfs, axis=0)
        
        # Filtrer les valeurs négligeables pour alléger le graphique
        df_to_plot = df_to_plot.loc[df_to_plot['GWP'].abs() > 1e-3]

        # ---------- Plot ----------
        # Utilisation de px.bar comme dans graph_C_inv_phase
        fig = px.bar(
            df_to_plot,
            x='Phases',
            y='GWP',
            color='Category',
            # facet_row='Type', # Optionnel si vous n'avez qu'un seul type ici
            title=self.case_study + ' - GWP Construction by phase',
            color_discrete_map=self.color_dict_full
        )

        fig.update_xaxes(
            categoryorder='array',
            categoryarray=sorted(df_to_plot['Phases'].unique())
        )
        
        # Ajout du nom de l'axe Y pour la clarté
        fig.update_yaxes(title_text='GWP [ktCO2e]')

        pio.show(fig)
        fig.write_image(self.outdir + "Gwp_constr_phase.pdf", width=1200, height=550)
        plt.close()
    ...
    def graph_co2_captured_resource(self, plot=True):
        pio.renderers.default = 'browser'
    ...
    def graph_gwp_combined_breakdown(self, plot=True):
        pio.renderers.default = 'browser'
        
        # --- 1. GWP Opérationnel (Source: Gwp_breakdown [ktCO2eq]) ---
        results_gwp = self.ampl_collector['Gwp_breakdown'].copy()
        df_op = results_gwp[['GWP_op']].copy()
        df_op.rename(columns={'GWP_op': 'Value'}, inplace=True)
        df_op['Type'] = '1. Operational' 
        
        # --- 2. GWP Construction (Source: Gwp_breakdown [ktCO2eq]) ---
        # On prend le flux annuel (pas par phase) pour l'aire empilée
        df_constr = results_gwp[['GWP_constr']].copy()
        df_constr.rename(columns={'GWP_constr': 'Value'}, inplace=True)
        df_constr['Type'] = '2. Construction'
        
        # --- 3. CO2 Capturé (Source: Resources [ktCO2]) ---
        results_res = self.ampl_collector['Resources'].copy()
        
        # Sécurité : Vérifier si CO2_CAPTURED existe
        if 'CO2_CAPTURED' in results_res.index.get_level_values('Resources'):
            df_cap = results_res.loc[results_res.index.get_level_values('Resources') == 'CO2_CAPTURED'].copy()
            df_cap = df_cap.reset_index()
            df_cap.rename(columns={'Res': 'Value', 'Resources': 'Elements'}, inplace=True)
            df_cap.set_index(['Years', 'Elements'], inplace=True)
        else:
            # Créer un DataFrame vide si pas de capture (pour éviter le crash)
            df_cap = pd.DataFrame(columns=['Value', 'Elements'])
            
        # On met la valeur en NÉGATIF pour l'affichage graphique (flux sortant du bilan)
        df_cap['Value'] = -df_cap['Value']
        df_cap['Type'] = '3. Captured'
        
        # --- 4. Fusion et Conversion ---
        dfs = [df_op, df_constr, df_cap]
        df_all = pd.concat(dfs).reset_index()
        
        # Nettoyage
        df_all.dropna(subset=['Value'], inplace=True)
        df_all['Years'] = df_all['Years'].str.replace('YEAR_', '')
        df_all['Elements'] = df_all['Elements'].astype(str)
        
        # --- CONVERSION D'UNITÉS : kt -> Mt ---
        # On divise par 1000 pour passer de ktCO2eq à MtCO2eq
        df_all['Value'] = df_all['Value'] / 1000
        unit = "MtCO2eq/y" 

        # Filtrage pour la lisibilité (supprime les poussières < 0.001 Mt)
        df_all = df_all[df_all['Value'].abs() > 0.001]
        ...
        if plot:
            ...
            fig = px.area(
                ...
                color='Elements', 
                facet_row='Type', 
                title=self.case_study + ' - Global GWP Breakdown',
                color_discrete_map=self.color_dict_full,
                category_orders={
                    "Type": ["1. Operational", "2. Construction", "3. Captured"]
                }
            )
            ...
            # Échelles indépendantes (matches=None) pour bien voir les variations de chaque section
            fig.update_yaxes(matches=None, showticklabels=True)
            
            # Mise à jour des titres des axes Y pour afficher l'unité
            # On boucle sur les axes pour mettre l'unité partout
            fig.update_yaxes(title_text=f"Emissions [{unit}]")
            
            # Axe X
            fig.update_xaxes(categoryorder='array', categoryarray=sorted(df_all['Years'].unique()))
            
            # Nettoyage des étiquettes de facettes (enlève "Type=")
            fig.for_each_annotation(lambda a: a.update(text=a.text.split("=")[-1]))

            pio.show(fig)
            
            # Sauvegarde PDF
            fig.write_image(self.outdir + "GWP_Combined_Breakdown.pdf", width=1200, height=800)
            plt.close()
    ...
    def graph_cost(self):
        ...
        c_inv = results[['C_inv']].fillna(0)
        c_op = results[['C_op']].fillna(0)
        c_maint = results[['C_maint']].fillna(0)
    ...
    def graph_cost_op_phase_categorie(self, ampl_collector = None, plot=True):
        pio.renderers.default = 'browser'
        category = self.category
    ...
    def graph_C_inv_phase_tech_stacked(self):
        pio.renderers.default = 'browser'
    ...
    def graph_C_op_phase_tech_stacked(self):
        pio.renderers.default = 'browser'
        
        # --- Load C_op_phase_tech (MultiIndex: Phases, Technologies) ---
        results = self.ampl_collector['C_op_phase_tech'].copy()
        
        # --- Create dataframe and filter low values per phase ---
        df_to_plot = pd.DataFrame(index=results.index, columns=results.columns)
        for p in results.index.get_level_values(0).unique():
            temp = results.loc[results.index.get_level_values('Phases') == p, 'C_op_phase_tech']  
            temp = self._remove_low_values(temp, threshold=0.001)
            df_to_plot.update(temp)
        ...
        df_to_plot.dropna(how='all', inplace=True)
        df_to_plot.reset_index(inplace=True)
        df_to_plot.rename(columns={"C_op_phase_tech": "Cost"}, inplace=True)
        df_to_plot['Technologies'] = df_to_plot['Technologies'].astype("str")
        df_to_plot['Phases'] = df_to_plot['Phases'].astype("str")
        
        # --- Create area chart (flow style) ---
        fig = px.area(
            df_to_plot, 
            x='Phases', 
            y='Cost',
            color='Technologies',
            title=self.case_study + ' - Operational Cost by Phase and Technology',
            color_discrete_map=self.color_dict_full,
            labels={'Cost': 'Operational Cost [M€]', 'Phases': 'Phase'}
        )
    ...
    def graph_C_op_phase_res_flow(self):
        pio.renderers.default = 'browser'
        
        # --- Load C_op_phase_res (MultiIndex: Phases, Resources) ---
        results = self.ampl_collector['C_op_phase_res'].copy()
        
        # --- Create dataframe and filter low values per phase ---
        df_to_plot = pd.DataFrame(index=results.index, columns=results.columns)
        for p in results.index.get_level_values(0).unique():
            temp = results.loc[results.index.get_level_values('Phases') == p, 'C_op_phase_res']  
            temp = self._remove_low_values(temp, threshold=0.001)
            df_to_plot.update(temp)
        ...
        df_to_plot.dropna(how='all', inplace=True)
        df_to_plot.reset_index(inplace=True)
        df_to_plot.rename(columns={"C_op_phase_res": "Cost"}, inplace=True)
        df_to_plot['Resources'] = df_to_plot['Resources'].astype("str")
        df_to_plot['Phases'] = df_to_plot['Phases'].astype("str")
        ...
        fig = px.area(
            ...
            y='Cost',
            color='Resources',
            title=self.case_study + ' - Operational Cost by Phase and Resource',
            color_discrete_map=self.color_dict_full,
            labels={'Cost': 'Operational Cost [M€]', 'Phases': 'Phase'}
        )
    ...
    def graph_C_inv_phase(self): 
        pio.renderers.default = 'browser'

        # --- Load results ---
        c_inv_tech = self.ampl_collector['C_inv_phase_tech'].copy()
        dfs = []
        # ---------- CAPEX per technology ----------
        df_inv = c_inv_tech.copy().reset_index()
        df_inv = df_inv.rename(columns={'C_inv_phase_tech': 'Cost'})
        df_inv['Type'] = 'C_inv (tech)'
        df_inv['Category'] = df_inv['Technologies']
        dfs.append(df_inv[['Phases','Cost','Category','Type']])


        # ---------- Concatenate ----------
        df_to_plot = pd.concat(dfs, axis=0)
        df_to_plot = df_to_plot.loc[df_to_plot['Cost'].abs() > 1e-3]
        ...
        fig = px.bar(
            ...
            color='Category',
            facet_row='Type',
            title=self.case_study + ' - Cost investment by phase and technology',
            color_discrete_map=self.color_dict_full
        )
    ...
    @staticmethod
    def custom_fig(fig,title,yvals,xvals=[2020, 2022, 2024, 2026, 2028, 2030, 2032, 2034, 2036, 2038, 2040, 2042, 2044, 2046, 2048, 2050], ftsize=18,annot_text=None,annot_pos=None,
    ...
    def export_legend(self, element_list, filename="legend_only", title=None):
        """
        Génère un fichier PNG transparent contenant uniquement la légende 
        pour les éléments fournis dans la liste.
        ...
        # On ajoute une trace "fictive" pour chaque élément pour forcer son apparition dans la légende
        for elem in element_list:
            # On vérifie si on a une couleur pour cet élément
            color = self.color_dict_full.get(elem, 'grey') # Gris par défaut si pas trouvé
        ...
        fig.update_layout(
            ...
            legend=dict(
                title=title,
                font=dict(family="Raleway", size=18), # Même police que tes autres graphes
                bgcolor='rgba(0,0,0,0)', # Fond de la légende transparent
                itemsizing='constant',
                traceorder="normal"
            ),
        ...
        # Calcul approximatif de la hauteur nécessaire (25px par ligne + marge)
        height = len(element_list) * 30 + 50
        width = 300 # Largeur fixe, à ajuster si besoin

        # Sauvegarde
        file_path = self.outdir + filename + ".png"
        # On utilise write_image avec scale=2 pour une bonne résolution
        fig.write_image(file_path, width=width, height=height, scale=2)
        # plt.close() # Pas nécessaire ici car on n'utilise pas matplotlib, mais bonne pratique si mixte
